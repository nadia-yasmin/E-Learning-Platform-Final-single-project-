...
const Viewcart = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axiosInstance.get(`/showcart?learnerId=${userData._id}`);
    ...
    const removeCart = async (courseId) => {
        try {
            console.log("From remove cart learnerId courseId",userData._id,courseId)
    ...
    return (
        <ThemeProvider theme={defaultTheme}>
          ...
          <Container maxWidth="md" style={{ paddingTop: "20px", paddingBottom: "20px" }}>
            <ToastContainer/>
            <Typography variant="h4" style={{ color: "#00695f", textAlign: "center", fontSize: "1.5rem", marginBottom: "50px" }}>
              Shopping Cart
            </Typography>
              ...
              <>
                {cartData.length === 0 ? (
                  ...
                  <div className="shopping-cart">
                    ...
                    <Button variant="contained" color="primary" className="checkout" style={{ margin: "20px auto", display: "block" }}>
